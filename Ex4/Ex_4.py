    ...
    def __init__(self):
        ...
        for n_i, n in enumerate(U_store):
            if n == 0:
                ...
                self.n_top[n_i] = self.n_bottom[
                                      n_i] + n - 1  # n_top is the highest particle at the n level. n = n_t - n_b + 1
...
def energy_minus_probability(n, t, myu):
    ...
    p_n_minus = (g_n_minus / (np.exp((n - 1 - myu) / t) - 1)) / (
                g_n_plus / (np.exp((n + 1 - myu) / t) - 1) + g_n_minus / (np.exp((n - 1 - myu) / t) - 1))
    if n == 0:
        g_n_zeros = g_calculation(n)
        p_n_minus = (g_n_zeros / (np.exp((- myu) / t) - 1)) / (
                g_n_plus / (np.exp((n + 1 - myu) / t) - 1) + g_n_zeros / (np.exp((- myu) / t) - 1))
